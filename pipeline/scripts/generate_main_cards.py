...
BRAND_TO_BG = {
    ...
    **{f"ExitLag Gift Card | Tier {n}": "exitlag" for n in [1, 2, 3]},
    ...
    # === Мобильные операторы для категории "Тарифные планы" (44.xlsx) ===
    # eSIM бренды
    **{f"eSIM {c}": "esim" for c in [
        "France","United States","United Kingdom","Turkey","Hong Kong",
        "Italy","Spain","Germany","Georgia","Thailand","Saudi Arabia",
        "Japan","Uzbekistan","Serbia","Tunisia","Portugal","Greece","Singapore",
    ]},
    **{f"eSim {c}": "esim" for c in [
        "Indonesia","Qatar","Belgium","Cyprus","Egypt","China","Russia",
        "Austria","Montenegro",
    ]},
    "eSIM | Vietnam": "esim",
    "eSim United Arab Emirates": "esim",
    # Ваучеры SA
    **{f"Lebara Voucher | {c}": "lebara" for c in ["SA","UK","NL"]},
    **{f"Lebara | {c}": "lebara" for c in ["FR","DE"]},
    **{f"Lebara {c}": "lebara" for c in ["France","Spain"]},
    **{f"Zain Voucher | {c}": "zain" for c in ["SA","KW","JO"]},
    **{f"Zain {c}": "zain" for c in ["Bahrain"]},
    **{f"Mobily Voucher | {c}": "mobily" for c in ["SA"]},
    **{f"stc Voucher | {c}": "stc" for c in ["SA"]},
    **{f"Ooredoo Voucher | {c}": "ooredoo" for c in ["OM","KW"]},
    **{f"Ooredoo {c}": "ooredoo" for c in ["Algeria"]},
    **{f"Virgin Mobile Voucher | {c}": "virginmobile" for c in ["SA","AE"]},
    **{f"etisalat Voucher | {c}": "etisalat" for c in ["AE"]},
    **{f"etisalat {c}": "etisalat" for c in ["Egypt","United Arab Emirates"]},
    **{f"Etisalat {c}": "etisalat" for c in ["Afghanistan"]},
    **{f"du eVoucher | {c}": "du" for c in ["AE"]},
    **{f"FRiENDi Voucher | {c}": "friendi" for c in ["SA","OM"]},
    **{f"Salam Voucher | {c}": "salam" for c in ["SA"]},
    **{f"Syma Voucher | {c}": "syma" for c in ["FR"]},
    "Airalo Voucher": "airalo",
    # Африка/Азия
    **{f"Afghan Wireless {c}": "afghanwireless" for c in ["Afghanistan"]},
    **{f"ATOMA {c}": "atoma" for c in ["Afghanistan"]},
    **{f"Africell {c}": "africell" for c in ["Angola"]},
    **{f"Movicel {c}": "movicel" for c in ["Angola"]},
    **{f"Airtel {c}": "airtel" for c in ["Bangladesh"]},
    **{f"Banglalink {c}": "banglalink" for c in ["Bangladesh"]},
    **{f"Grameenphone {c}": "grameenphone" for c in ["Bangladesh"]},
    **{f"AIS {c}": "ais" for c in ["Thailand"]},
    **{f"Orange {c}": "orange" for c in ["Guinea"]},
}
